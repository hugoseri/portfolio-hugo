...
class Entry extends React.Component {
  ...
  render() {
    ...
    let img = (
      <div key={this.props.index} className="col-md-5">
        <img
          src={`${this.props.entry.img}`}
          alt=""
          className="img-fluid img_project"
        />
      </div>
    );
    let content = (
      <div
        key={this.props.index + this.props.total}
        className="col-md-7 mt-sm-20 text_project"
      >
        <h3>{this.props.entry.name}</h3>
        <h4>
          <i>
            {this.props.text.date_connector_start +
              " " +
              startDate +
              " " +
              this.props.text.date_connector_end +
              " " +
              endDate}
          </i>
        </h4>
        ...
        <div className="button-group-area">
          {this.props.entry.skills.map(function (entry, index) {
            return <SkillEntry key={index} index={index} entry={entry} />;
          })}
        </div>
      </div>
    );
